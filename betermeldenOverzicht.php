...
{
    $str = $_POST;
    $sth = $conn->prepare("SELECT * FROM beter order by voornaam");
...
{
    $str = $_POST;
    $sth = $conn->prepare("SELECT * FROM beter order by achternaam");
    $row = $sth->setFetchMode(PDO:: FETCH_OBJ);
    $sth->execute();
...
elseif (isset ($_POST['achternaam'])){
    $str = $_POST;
    $sth = $conn->prepare("SELECT * FROM beter order by klas");
    $row = $sth->setFetchMode(PDO:: FETCH_OBJ);
    $sth->execute();
}
